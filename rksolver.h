...
// Inputs: board[c][n] (must-use counts), rackAvail[c][n] EXTRA rack copies,
// jokerBoard (must place), jokerRack (extra). c in 0..3, n in 1..13.
// Fills placeBuf ("Black: 1 3 ...") and setsBuf (arrangement). Returns rack tiles
// placed, or -1 if the board can't be validly arranged.
static int rk_solve(int board[NCOL][NNUM + 1], int rackAvail[NCOL][NNUM + 1],
                    int jokerBoard, int jokerRack,
                    char *placeBuf, int placeCap, char *setsBuf, int setsCap) {
    buildTriples();
    memset(lo, 0, sizeof(lo)); memset(hi, 0, sizeof(hi));
    for (int c = 0; c < NCOL; c++) for (int n = 1; n <= NNUM; n++) {
        lo[c][n] = board[c][n];
        hi[c][n] = board[c][n] + rackAvail[c][n];
    }
    jokLo = jokerBoard; jokHi = jokerBoard + jokerRack;
    gBoardNumbered = 0;
    for (int c = 0; c < NCOL; c++) for (int n = 1; n <= NNUM; n++) gBoardNumbered += lo[c][n];

    stridePerNum = 1; for (int c = 0; c < NCOL; c++) stridePerNum *= gTriN;
    stridePerNum *= (jokHi + 1);
    for (int i = 0; i <= NNUM + 1; i++) {
        memo[i] = (int *)malloc(sizeof(int) * stridePerNum);
        for (long j = 0; j < stridePerNum; j++) memo[i][j] = NEG - 1;
    }
    int start[NCOL] = {0,0,0,0};
    int best = solve(1, start, 0);
    int ret;
    if (best <= NEG) { ret = -1; if (setsBuf && setsCap) setsBuf[0] = 0; if (placeBuf && placeCap) placeBuf[0] = 0; }
    else {
        reconstruct();
        if (setsBuf && setsCap) { int n = setsLen < setsCap - 1 ? setsLen : setsCap - 1; memcpy(setsBuf, setsOut, n); setsBuf[n] = 0; }
        if (placeBuf && placeCap) {
            int p = 0;
            for (int c = 0; c < NCOL; c++) {
                int any = 0;
                for (int n = 1; n <= NNUM; n++) {
                    int add = realUsed[c][n] - lo[c][n];
                    for (int x = 0; x < add && p < placeCap - 8; x++) {
                        if (!any) { p += snprintf(placeBuf + p, placeCap - p, "%s:", CNAME[c]); any = 1; }
                        p += snprintf(placeBuf + p, placeCap - p, " %d", n);
                    }
                }
                if (any && p < placeCap - 2) p += snprintf(placeBuf + p, placeCap - p, "\n");
            }
            placeBuf[p] = 0;
        }
        ret = best - gBoardNumbered;
    }
    for (int i = 0; i <= NNUM + 1; i++) { free(memo[i]); memo[i] = NULL; }
    return ret;
}
